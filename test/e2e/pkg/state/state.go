...
func (s *Scenario) CaptureRoundTrip(method, scheme, hostname, path string, headerInfo net_http.Header, doRedirect bool) error {
	var location string
	...
	capturedRequest, capturedResponse, err := http.CaptureRoundTrip(method, scheme, hostname, path, location, headerInfo, doRedirect)
	if err != nil {
		return err
	}
